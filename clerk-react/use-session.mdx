...
| `options.treatPendingAsSignedOut?` | `boolean`                                            | Determines if pending sessions are considered as signed-out state. Defaults to `true`. |
...
| `session` | `null` | Holds the current active session for the user. |
</Tab>
<Tab>
| Name | Type | Description |
| ------ | ------ | ------ |
...
| `session` | [`SignedInSessionResource`](/docs/references/javascript/session) | Holds the current active session for the user. |
</Tab>
</Tabs>
