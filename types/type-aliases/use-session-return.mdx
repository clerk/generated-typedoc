> **UseSessionReturn**: \{ `isLoaded`: `false`; `isSignedIn`: `undefined`; `session`: `undefined`; \} \| \{ `isLoaded`: `true`; `isSignedIn`: `false`; `session`: `null`; \} \| \{ `isLoaded`: `true`; `isSignedIn`: `true`; `session`: [`SignedInSessionResource`](signed-in-session-resource.mdx); \}
