...
`MaybePromise`\<[`InferAuthObjectFromTokenArray`](infer-auth-object-from-token-array.mdx)\<`T`, `SessionAuthObject`, `MachineAuthObject`\<`Exclude`\<`T`\[`number`\], `"session_token"`\>\>\>, `ReturnsPromise`\>
