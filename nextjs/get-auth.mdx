...
| `opts?`   | <code>\{ secretKey?: string; \}</code> & <code>\{ treatPendingAsSignedOut?: boolean; \}</code> | -                           |

## Returns

`AuthObject` — The `Auth` object. See the [Auth reference](/docs/references/backend/types/auth-object) for more information.
